...
```
# Explanation
1. dp[i][j]表示t[0..i]在s[0..j]中出现的次数
2. 如果t[i]==s[j]则dp[i][j] = dp[i][j-1]+dp[i-1][j-1],即t[0..i]在s[0..j-1]中出现的次数加上t[0..i-1]在s[0..j-1]中出现的次数，因为每个t[0..i-1]在s[0..j-1]中出现的位置加上s[j]又可以组成新的t[0..i]。
3. 类似LCS的动态规划
# Conclusion